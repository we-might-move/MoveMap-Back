...
>
> **ES p99 미달에 대한 솔직한 평가**: p95는 SLO(500ms) 대비 37배 여유가 있지만, p99(2.49s)·max(6.56s)는 여전히 SLO를 벗어난다 — 소수의 요청이 크게 튄다는 뜻이다. 원인은 이 문서에서 추가로 프로파일링하지 않았다(확인 필요). 후보로는 ES 클라이언트 커넥션 풀 크기, JVM GC 정지, 혹은 30 RPS가 ES 컨테이너(2 CPU)에도 순간적 큐잉을 만들 가능성 등이 있으나 **추측일 뿐 검증하지 않았다.**
...
- **완전히 해결됐다고는 말하지 않는다.** ES 조건에서도 30 RPS(추정 피크의 최대 10배에 달하는 의도적 과부하) p99는 여전히 SLO(1s)를 넘는다(2.49s). "AS-IS 대비 압도적으로 개선"과 "SLO를 항상 만족한다"는 다른 주장이며, 이 문서는 전자만 실측으로 뒷받침한다.
...
4. **각 조건을 1회씩만 측정했다**(AS-IS의 closed-model처럼 3회 반복·중앙값을 취하지 않았다). 특히 facilities의 med 소폭 증가(§3-3)처럼 작은 차이는 반복 측정 없이는 "개선/악화"를 단정하기 어렵다 — 표에는 실측값 그대로 적었을 뿐 반복 검증은 하지 않았다.
5. **ES p99/max tail의 원인은 미조사 상태다**(§3-2). "추측"으로 채우지 않고 "확인 필요"로 남긴다.
